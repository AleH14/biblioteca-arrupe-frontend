...
const EjemplaresManager = React.memo(
  ({
    ...
    const agregarEjemplar = () => {
      const nuevoId =
        ejemplares.length > 0
          ? Math.max(...ejemplares.map((e) => e.id)) + 1
          : 1;
      onAgregarEjemplar(nuevoId);
    };

    //handre de donado/comprado
    const handleTipoChange = (index, value) => {
      const donado = value === "donado";
      onEjemplarChange(index, "donado", donado);
      // Resetear campos cuando cambia el tipo
      if (donado) {
        onEjemplarChange(index, "precio", "");
      } else {
        onEjemplarChange(index, "origen", "");
      }
    };
    ...
    const ejemplaresConValores = ejemplares.map((ejemplar) => ({
      ...
      donado: ejemplar.donado ?? null,
      origen: ejemplar.origen || "",
            ...
            <tbody>
              {ejemplaresConValores.map((ejemplar, index) => (
                <tr key={ejemplar.id}>
                  ...
                  {/* Tipo (Donado/Comprado) */}
                  <td>
                    <select
                      value={
                        ejemplar.donado === null
                          ? ""
                          : ejemplar.donado
                          ? "donado"
                          : "comprado"
                  ...
                  {/* Precio/Origen */}
                  <td>
                    {ejemplar.donado === false ? (
                      <input
                        type="number"
                        value={ejemplar.precio}
                        onChange={(e) =>
                          handleEjemplarChange(index, "precio", e.target.value)
                        }
                        className={styles.tablaInput}
                        placeholder="0.00"
                        step="0.01"
                        min="0"
                      />
                    ) : ejemplar.donado === true ? (
                      <input
                        type="text"
                        value={ejemplar.origen}
                        onChange={(e) =>
                          handleEjemplarChange(index, "origen", e.target.value)
                        }
                        className={styles.tablaInput}
                        placeholder="Origen del libro"
                      />
                    ) : (
                      <span>
                        Seleccione tipo
